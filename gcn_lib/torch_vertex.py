...
class DyGraphConv2d(GraphConv2d):
    """
    Dynamic graph convolution layer with leap-ahead graph construction support
    """
    ...
    def forward(self, features, relative_pos=None, features_for_graph=None):
        """
        features: The current feature tensor to be convolved (FU)
        features_for_graph: Feature tensor to construct the graph from (GC),
                            usually FC output from the previous layer for GRAPHLEAP.
        """
        B, C, H, W = features.shape
        
        # Use features_for_graph for graph construction if given (GraphLeap design)
        ...
        edge_index = self.dilated_knn_graph(features_graph, y, relative_pos)
...
class Grapher(nn.Module):
    """
    Grapher module with graph convolution and fc layers supporting leap-ahead graph construction
    """
    ...
    def forward(self, features, features_for_graph=None):
        """
        features: current input features for forward (FU)
        features_for_graph: features used for graph construction (GC) - usually previous FC1 output
        """
        _tmp = features
        ...
        # Pass features_for_graph to dynamic graph convolution for GRAPHLEAP
        features_after_gc = self.graph_conv(features_after_fc1, relative_pos, features_for_graph)
        features_after_fc2 = self.fc2(features_after_gc)
        output = self.drop_path(features_after_fc2) + _tmp

        # Return output and the FC1 output for use as features_for_graph in the next layer
