# vig.py - Visual Graph Neural Network with GraphLeap profiling hooks
...
default_cfgs = {'gnn_patch16_224': _cfg(crop_pct=0.9, input_size=(3, 224, 224),
...
class CPUGraphConstructorThread:
    ...
    def __init__(self, grapher_list, device='cuda', n_blocks=12):
        ...
        self.gcn_model_profiler = gcn_model_profiler
    ...
    def _construct_graph_cpu(self, features_cpu, layer_idx):
        ...
        if self.gcn_model_profiler is not None:
            self.gcn_model_profiler("graph_construction", graph_duration)
        return edge_index
...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.profile = {
            'graph_construction_total': 0.0,
            'feature_update_total': 0.0,
            'h2d_total': 0.0,
            'd2h_total': 0.0
        }
        ...
        if self.use_graphleap:
            self.cpu_graph_constructor = CPUGraphConstructorThread(
                ...
                gcn_model_profiler=self._profile_callback
            )
    ...
    def _profile_callback(self, phase, elapsed):
        if phase == "graph_construction":
            self.profile['graph_construction_total'] += elapsed
    ...
    def forward(self, inputs):
        ...
        if x.shape[2:] != pos_embed.shape[2:]:
            # Interpolate positional embedding spatially
            pos_embed = torch.nn.functional.interpolate(
                pos_embed, size=x.shape[2:], mode="bicubic", align_corners=False
            )
        x = x + pos_embed
        B, C, H, W = x.shape
    ...
    def _forward_graphleap(self, x):
        ...
        x = ffn_0(x)
        self.profile['feature_update_total'] += time.time() - gpu_feat_start
