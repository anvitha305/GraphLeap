...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        if opt.use_dilation:
            for i in range(self.n_blocks):
                ...
                block_list.append(Seq(grapher, ffn))
        else:
            for i in range(self.n_blocks):
    ...
    def forward(self, inputs):
        ...
        features_for_graph = None  # Store the features for graph construction

        # Iterate through blocks (Grapher+FFN)
        for i in range(self.n_blocks):
            grapher, ffn = self.backbone[i]
            if i == 0:
                # First block uses initial embedding for graph construction
                x, features_for_graph = grapher(x, features_for_graph=None)
            else:
                # Subsequent blocks use previous FC1 output for graph construction
                x, features_for_graph = grapher(x, features_for_graph=features_for_graph)
